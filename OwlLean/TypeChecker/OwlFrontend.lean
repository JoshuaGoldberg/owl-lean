...
def doTc (n : TSyntax `ident) (s : Sequent) := do
  ...
  | .ok tres => do
    ...
    emitDefinition n.getId (toExpr $ s.e)
  | .err e => do
    logError s!"err: {e.2}"
    match e.1 with
    | .none => PURE
...
def elabLabelEntries' (stx : List (TSyntax `label_entry)) (L : TCtx) (ctx : lbl_ctx (ScopeMap.ofList [L.length])) (corrs : corr_ctx (ScopeMap.ofList [L.length])): TermElabM ((L' : TCtx) × lbl_ctx (ScopeMap.ofList [L'.length]) × corr_ctx (ScopeMap.ofList [L'.length])) :=
  ...
  | e :: es => do
    ...
    | .new_label n cs l => do
      let l' : label ((ScopeMap.ofList [L.length + 1])) := l.rename (((ScopeMap.ofList [L.length]).embed #L))
      let L' := L ++ [n]
      let ctx' : lbl_ctx (ScopeMap.ofList [L'.length]) :=
       ((ctx.map fun _ (n, (c, l, ty)) => (n, (c, l.rename (((ScopeMap.ofList [L.length]).embed #L)), ty))).snoc (n, (cs, l', .MetaLbl))).cast (by simp [L']) (by simp [L']; rw [List.length_append]; simp)
      let corrs' : corr_ctx (ScopeMap.ofList [L'.length]) :=
        corrs.map fun c => c.rename ((((ScopeMap.ofList [L.length]).embed #L)).castR (by
           simp [L']
           rw [List.length_append]
           simp
           apply ScopeMap.ext
           intros x
           rw [ScopeMap.ofList_get]
           simp; grind
           ))
...
def elabTyVarEntries' (stx : List (TSyntax `ty_var_entry)) (L : TCtx) (R : TCtx) (D : TCtx) (ctx : ty_var_ctx (ScopeMap.ofList [L.length, R.length, D.length])) : TermElabM ((D' : TCtx) × ty_var_ctx (ScopeMap.ofList [L.length, R.length, D'.length])) :=
  ...
  | e :: es => do
    ...
    let t' := t.rename (((ScopeMap.ofList [L.length, R.length, D.length]).embed #Ty))
    let D' := D ++ [n]
    let ctx' : ty_var_ctx (ScopeMap.ofList [L.length, R.length, D'.length]) :=
       ((ctx.map fun _ (n, t) => (n, t.rename (((ScopeMap.ofList [L.length, R.length, D.length]).embed #Ty)))).snoc (n, t')).cast (by simp [D']) (by
         simp [D']
         congr 2
         apply ScopeMap.ext
         intros x
...
def elabTmEntries' (stx : List (TSyntax `tm_entry)) (L : TCtx) (R : TCtx) (D : TCtx) (M : TCtx) (ctx : tm_ctx (ScopeMap.ofList [L.length, R.length, D.length, M.length])) : TermElabM ((G' : TCtx) × tm_ctx (ScopeMap.ofList [L.length, R.length, D.length, G'.length])) :=
  ...
  | e :: es => do
    ...
    let ctx' : tm_ctx (ScopeMap.ofList [L.length, R.length, D.length, M'.length]) :=
      (ctx.snoc (n, t)).cast (by simp [M']; grind) (by
       congr 1
      )
...
def printSeq (seq : Sequent) : TermElabM Unit := do
  println! "L: {seq.l}"
  println! "R: {seq.r}"
  println! "D: {seq.d}"
  println! "M: {seq.m}"
  println! "ref_vars: {seq.ref_vars}"
  println! "Phi: {seq.Phi}"
  println! "Psi: {seq.Psi}"
  println! "Delta: {seq.Delta}"
  println! "Theta: {seq.Theta}"
  println! "Gamma: {seq.Gamma}"
